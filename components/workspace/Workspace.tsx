 ...
 *
 * This is a client component because the whole workspace is interactive. The
 * signed-in identity is resolved on the server in `app/page.tsx` and arrives as
 * a prop — the browser is never asked who the user is, and never gets to
 * answer.
 */
        ...
        <div className="sidebar-footer">
          ...
          >
            <span className="avatar teacher-avatar">
              {shellUser.initials || <UserRound size={16} />}
            </span>
            <span>
              <strong>{shellUser.displayName}</strong>
              <small>{shellUser.subtitle}</small>
            </span>
        ...
        {panel.mode === "notifications" ? (
          ...
          <div className="destination-body">
            <div className="destination-profile">
              <span className="avatar teacher-avatar">
                {shellUser.initials || <UserRound size={18} />}
              </span>
              <div>
                <strong>{shellUser.displayName}</strong>
                <span>{shellUser.subtitle}</span>
              </div>
            </div>

            {/* A plain form posting to a server action, so signing out clears
                the cookie on the server. Clearing it client-side would leave
                the next server render still holding a valid session. */}
            {shellUser.signedIn && (
              <form action={signOutAction} className="destination-section">
